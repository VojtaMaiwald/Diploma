...
MINIMALISTIC = False
LEARNING_RATE = 0.01
ENDING_STRING = ("AUGFULL" if AUGMENT else "") + ("_SHUFFLE" if SHUFFLE else "") + ("_MINI" if MINIMALISTIC else "")
...
#def load_model(existingModelPath = None):
	if existingModelPath != None:
		...
		with strategy.scope():
			base_model = MobileNetV3Large(input_shape = IMAGE_SHAPE, weights = None, minimalistic = MINIMALISTIC, alpha = ALPHA, include_top = False)
			x = base_model.output
