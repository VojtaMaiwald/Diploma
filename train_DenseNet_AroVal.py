...
MODEL_NAME = f"DenseNet121_AroVal_E{EPOCHS}_B{BATCH_SIZE}_SGD{LEARNING_RATE}_{ENDING_STRING}"
...
def load_model(existingModelPath = None):
	if existingModelPath != None:
		...
		#with strategy.scope():
			base_model = DenseNet121(input_shape = IMAGE_SHAPE, weights = None, include_top = False)
			x = base_model.output
