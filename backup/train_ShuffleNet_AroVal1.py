...
SHUFFLE = True
LEARNING_RATE = 0.001
CHANNELS = 500 # must be divisible by (4 * num of classes)
DROPOUT = 0.2
ENDING_STRING = ("AUGFULL" if AUGMENT else "") + ("_SHUFFLE" if SHUFFLE else "")
MODEL_NAME = f"ShuffleNet_AroVal_E{EPOCHS}_B{BATCH_SIZE // 3}_Channels{CHANNELS}_Adam{LEARNING_RATE}_{ENDING_STRING}"
...
#def load_model(existingModelPath = None):
	if existingModelPath != None:
		...
		with strategy.scope():
			base_model = ShuffleNet(start_channels = CHANNELS, input_shape = IMAGE_SHAPE, include_top = False)
			x = base_model.output
