...
MODEL_NAME = f"ShuffleNetV2_AroVal_E{EPOCHS}_B{BATCH_SIZE // 3}_SC{SCALE_FACTOR}_BOTTLENECK{BOTTLENECK}_Adam{LEARNING_RATE}_{ENDING_STRING}"
...
#def load_model(existingModelPath = None):
	if existingModelPath != None:
		...
		with strategy.scope():
			base_model = ShuffleNetV2(input_shape = IMAGE_SHAPE, scale_factor = SCALE_FACTOR, bottleneck_ratio = BOTTLENECK, include_top = False)
			x = base_model.output
