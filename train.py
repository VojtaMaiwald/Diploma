...
def load_model(existingModelPath = None):
	#model = MobileNetV2(input_shape = (224, 224, 3), alpha = 1.0, include_top = True, weights = "imagenet", input_tensor = None, pooling = None, classes = 0, classifier_activation = "softmax")
	
	if existingModelPath != None:
		model = tf.keras.models.load_model(existingModelPath)
	else:
		model = MobileNetV2(classes = 8, weights = None)
		#model = MobileNetV2(input_shape = IMAGE_SHAPE, include_top = False, classes = 8)
		#model = MobileNetV2(input_shape = IMAGE_SHAPE, include_top = False, weights = "imagenet", classes = 8)
...
if __name__ == "__main__":
	...
	history = model.fit(
		...
		callbacks = [
			#ModelCheckpoint(MODEL_PATH + "mobilenet_" + f'{DONE_EPOCHS+epoch:02d}_{val_loss:.3f}_T.tf', monitor = 'val_acc',
			ModelCheckpoint(MODEL_PATH + "mobilenet_" + '{epoch:02d}_{val_loss:.3f}_T.tf', monitor = 'val_acc',
							save_best_only = False,
							save_weights_only = False,
							save_format = 'tf'),
			CSVLogger(MODEL_PATH + 'log_classification.csv', append = True, separator = ';')],
		workers = 12,
	...
	model.save_weights(MODEL_PATH + '_weights', save_format = 'tf', overwrite = True)
	model.save(MODEL_PATH + '_full_model', save_format = 'tf', overwrite = True)
	print(" ***** ENDING ***** ")
